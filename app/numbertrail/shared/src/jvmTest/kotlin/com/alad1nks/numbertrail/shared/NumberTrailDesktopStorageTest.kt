...
class NumberTrailDesktopStorageTest {
    @Test
    fun numberTrailUsesAProductLocalDurablePath() {
        val userHome = File("build/test-home").absolutePath
        val numberTrailPath = numberTrailDataStorePath(userHome)
        val genericOquTurboPath = File(System.getProperty("java.io.tmpdir"), "oquturbo.preferences_pb").absolutePath

        assertEquals(
            File(userHome, ".numbertrail/$NUMBER_TRAIL_DATA_STORE_FILE_NAME").absolutePath,
            numberTrailPath,
        )
        assertNotEquals(genericOquTurboPath, numberTrailPath)
        assertNotSame(StorageDataStoreModule, getPlatformModules().single())
    }

    @Test
    fun numberTrailModulePersistsPreferencesThroughItsSingletonDataStore() =
        runTest {
            val userHome = Files.createTempDirectory("numbertrail-storage-test").toFile()
            val dataStoreFile = File(numberTrailDataStorePath(userHome.absolutePath))
            val koinApplication =
                startKoin {
                    modules(numberTrailStorageDataStoreModule(userHome.absolutePath))
                }
            try {
    ...
    private companion object {
        const val TEST_RECORD_KEY = "number_trail_test_record"
    }
}
